...
├── leave/              # 연차유급휴가 (신청→승인, 잔여 계산)
├── payroll/            # 급여 계산 (수당·공제·퇴직금 추정)
...
- **휴일근로 가산** (제56조 2항, 상시 5인 이상만): 법정 유급휴일(관공서 공휴일·대체공휴일·근로자의 날)
  근로에 8시간 이내 50%, 초과분 100% 가산. 연장근로와 중복 가산하지 않음.
  공휴일에 스케줄이 있었지만 쉰 것은 개근 판정에서 제외 (주휴수당 유지)
- **유급휴일수당** (제55조 2항·근로자의날법): 유급휴일이 소정근로일(스케줄 존재)이면 근무 여부와
  무관하게 소정근로시간(일 8시간 한도) × 현재 시급 지급. 근무 시 근로임금+가산과 합쳐 통상 2.5배.
  5인 미만 사업장은 근로자의 날만 해당 (가산 없이 유급분·근로임금)
- **약정 주휴일**: 직원별 주휴일 요일(weeklyRestDay) 지정 시 그날 근무는 휴일근로 가산(5인 이상),
  쉼은 결근 아님. 유급분은 주휴수당이 담당하므로 가산만 지급
- **연차유급휴가** (제60조, 5인 이상): 1년 미만 개근 월당 1일(최대 11), 1년 이상 15일+2년마다 1일(최대 25).
  직원 신청 → 사장 승인. 승인된 연차는 유급(annualLeavePay)이며 개근·결근 판정에서 제외.
  단순화: 이월 없음, 1년 이상의 80% 출근율 요건 미판정
- **퇴직금 추정** (퇴직급여보장법): 1일 평균임금(3개월 임금총액÷총일수, 통상임금 보정) × 30 × 근속연수.
  자격: 1년 이상 + 최근 4주 평균 주 15시간
- **4대보험 공제 추정**: 근로자 부담 요율(환경변수, 매년 갱신) 적용한 실수령 추정.
  가입 대상 판정·소득세는 미포함
  - 공휴일 데이터: `KoreanHolidayCalendar`에 2025~2027 하드코딩 (설·추석·부처님오신날·대체공휴일 포함).
    **매년 월력요항 발표 후 다음 해 데이터 추가 필수**, 선거일·임시공휴일은 지정 시 수동 추가
- **최저임금**: 시급 정책 생성/수정 시 최저시급(`MINIMUM_WAGE`, 기본 10,320원) 미달 차단
- 단순화: 조회 구간이 주 중간을 자르면 그 주는 잘린 부분만 집계 (정산은 주 단위로 조회 권장),
  직원별 약정 주휴일(요일) 근무 가산은 미구현 (주휴일 지정 데이터 없음).
  유급휴일수당의 시급은 현재 정책 시급, 소정근로시간은 스케줄 전체 길이 사용.
  휴게 부여 의무(제54조, 4시간당 30분)는 기록 기반 시스템 특성상 강제하지 않음
